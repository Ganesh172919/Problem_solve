...
function rowToPost(row: PostRow): Post {
  return {
    ...
    sourceReferences: safeJsonParse<string[]>(row.sourceReferences, []),
...
export function createPost(post: Omit<Post, 'id' | 'createdAt' | 'updatedAt'>): Post {
  ...
  // FTS index is kept in sync by triggers (posts_fts_ai/posts_fts_ad/posts_fts_au)
  // defined in db/index.ts. No manual FTS insert needed here.
...
export function deletePost(id: string): boolean {
  ...
  // FTS index is kept in sync by the posts_fts_ad trigger
...
export function searchPosts(
  ...
  try {
    ...
    // FTS not available or malformed query — fall back to LIKE search
    logger.debug('FTS search failed, falling back to LIKE', { query: trimmed, error: String(err) });
