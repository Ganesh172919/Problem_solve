...
class AIDrivenDocumentation {
  ...
  private readonly searchIndex = new Map<string, string>(); // pageId → searchable text
  ...
  private renderApiDocMarkdown(serviceId: string, endpoints: ApiEndpointDoc[]): string {
    ...
    for (const ep of endpoints) {
      ...
      if (ep.deprecated) lines.push('> ⚠️ **Deprecated** — use the newer API version.');
      lines.push('');
  ...
  private renderChangelogMarkdown(serviceId: string, entries: ChangelogEntry[]): string {
    ...
    for (const [version, vEntries] of Object.entries(byVersion).sort().reverse()) {
      ...
      for (const e of vEntries) {
        const prefix = e.breaking ? '⚠️ **BREAKING** ' : '';
        const icon = { feature: '✨', bugfix: '🐛', breaking_change: '💥', deprecation: '⚡', security: '🔒', performance: '🚀', refactor: '♻️' }[e.type] ?? '';
  ...
  private buildQualitySuggestions(page: DocPage, freshness: number, completeness: number): string[] {
    ...
    if (freshness < 60) suggestions.push('Update content — last edited over 3 weeks ago');
    if (completeness < 50) suggestions.push('Expand content — consider adding more detail and context');
